...
function ldap_binding($user, $username="", $password){
   if ( $username == 'admin' ) { return $user; }
	...
	$ldappass = $password;	
	$_SESSION['userpass'] = base64_encode($password);
	...
	if ($ldapconn) {		
		...
		$ldapbind = ldap_bind($ldapconn, $ldaprdn, $ldappass) or die(wp_redirect( site_url()."/index.php?errmsg=INVALID"));
		$filter1 = "(".LDAP_USER_FILTER."=" . $samaccount . ")";
		...
		if ($entries) {
			$departments=array("QE&A Technology CoE");
			if (!in_array($entries[0]['department'][0],$departments)) {
				wp_redirect( site_url()."/index.php?errmsg=NOTCOE");	
				exit;
			}
		}
		if(isset($entries[0]['thumbnailphoto']))
			$_SESSION['lserverConnection'] = $ldapconn;
			$_SESSION['thumbnailphoto'] = 'data:image/png;base64,'.base64_encode($entries[0]['thumbnailphoto'][0]);
