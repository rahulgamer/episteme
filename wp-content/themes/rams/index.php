...
if (is_user_logged_in()){
 ...
 <a href="#inline1" id="fancyPopup" class="fancybox"></a>
 
 <div class="login-form" id="inline1" style="width:400px;display: none;">
    <div id="LoginTitleDiv">
             <div id="LoginWelcomeText">Welcome to Episteme!</div>
    </div>
	<?php
		if(isset($_GET['errmsg'])){
			echo "<div style='margin:10px;color:red;'>".constant($_GET['errmsg'])."</div>";
		}
	?>
      <form action="<?php echo get_option('home'); ?>/wp-login.php" method="post">
      <!-- p class="login_welcome">Welcome to Episteme</p --> 
       <label class="login_label">User ID</label> <input type="text" name="log" id="log" value="<?php echo wp_specialchars(stripslashes($user_login), 1) ?>" size="20" class="login-fields" onclick="this.value='';" onblur="this.value=!this.value?'':this.value;"/>
        <label class="login_label">Password</label><input type="password" name="pwd" id="pwd" value="" size="20" class="login-fields" onclick="this.value='';" onblur="this.value=!this.value?'':this.value;"/>
        <input type="submit" name="submit" value="Login" class="login-button" />
        <p class="login_p">*Please use Cognizant credentials to login.</p>
      </form>
</div>
...
<?php
	// Default Posts list display section
	// Commented by Prasanna on 18-09-2014
	/*
<div class="content">
...
<script type="text/javascript">
	jQuery('#fancyPopup').fancybox().trigger('click');
</script>	              	        
<?php get_footer(); ?>